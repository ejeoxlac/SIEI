...
def docx_pm_pv(addressed_to_entry, by_entry, subject_entry, namepm, model, serial, observation_docx_entry, item_values):
    ...
    doc_path = os.path.join(carpeta_temp, "temp_document_pm.docx")
...
def convertir_a_pdf(addressed_to_entry, by_entry, subject_entry, namepm, model, serial, observation_docx_entry, item_values):
    archivo_docx = docx_pm_pv (addressed_to_entry, by_entry, subject_entry, namepm, model, serial, observation_docx_entry, item_values)  # Generar el documento DOCX automáticamente
...
def mostrar_pdf(pdf_path, docx_path):
    ...
    for page_num in range(len(doc)):
        ...
        img_path = os.path.join("temp", f"temp_image_pm_{page_num}.png")  # Guardar en la carpeta temp
        pix.save(img_path)
